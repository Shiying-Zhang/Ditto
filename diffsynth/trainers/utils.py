...
def wan_parser():
    ...
    parser.add_argument("--sa_rope_mode", type=str, default="none", choices=["none", "fixed", "source", "memory", "source+memory"], help="Adaptive spatio-temporal RoPE mode.")
    parser.add_argument("--sa_rope_hidden_dim", type=int, default=128, help="Hidden dimension for learned SA-RoPE scale predictor.")
    parser.add_argument("--sa_rope_init_scale", type=float, default=0.1, help="Maximum learned scale delta at initialization range: 1 +/- init_scale.")
    parser.add_argument("--sa_rope_fixed_temporal_scale", type=float, default=1.0, help="Temporal RoPE scale for fixed SA-RoPE ablation.")
    parser.add_argument("--sa_rope_fixed_spatial_scale", type=float, default=1.0, help="Spatial RoPE scale for fixed SA-RoPE ablation.")
    parser.add_argument("--sa_rope_min_scale", type=float, default=0.5, help="Minimum clamped SA-RoPE scale.")
    parser.add_argument("--sa_rope_max_scale", type=float, default=1.5, help="Maximum clamped SA-RoPE scale.")
    parser.add_argument("--sa_rope_reg_weight", type=float, default=0.0, help="Optional regularization weight for keeping SA-RoPE scales near 1.")
    parser.add_argument("--sa_rope_head_roles_path", type=str, default=None, help="Optional JSON file with per-layer per-head spatial/temporal roles.")
    parser.add_argument("--sa_rope_default_head_role", type=str, default="alternate", choices=["alternate", "spatial", "temporal"], help="Fallback head role assignment when no JSON is provided.")
    parser.add_argument("--sa_rope_trace_head_roles_output", type=str, default=None, help="If set, run a short attention-density pass, save head roles JSON, and exit before training.")
    parser.add_argument("--sa_rope_trace_head_roles_samples", type=int, default=10, help="Number of samples used for SA-RoPE head role majority voting.")
    parser.add_argument("--sa_rope_trace_max_spatial_tokens", type=int, default=64, help="Max sampled spatial tokens per frame for attention-density head classification.")
    parser.add_argument("--identity_distill_weight", type=float, default=0.0, help="Weight for identity propagation inter-frame relation distillation.")
    parser.add_argument("--first_frame_mmd_weight", type=float, default=0.0, help="Weight for first-frame relation MMD consistency.")
    parser.add_argument("--identity_distill_max_tokens", type=int, default=256, help="Max spatial tokens sampled per frame for identity propagation distillation.")
    parser.add_argument("--easyvfx_frequency_mode", type=str, default="none", choices=["none", "fixed", "adaptive", "full"], help="EasyVFX-inspired frequency-driven training loss mode.")
    parser.add_argument("--easyvfx_low_ratio", type=float, default=0.25, help="Spatial low-pass cutoff ratio used by the EasyVFX frequency loss.")
    parser.add_argument("--easyvfx_temporal_low_ratio", type=float, default=0.35, help="Temporal low-pass cutoff ratio used before extracting temporal high-frequency residuals.")
    parser.add_argument("--easyvfx_low_weight", type=float, default=0.0, help="Weight for low-frequency clean-latent reconstruction.")
    parser.add_argument("--easyvfx_high_weight", type=float, default=0.0, help="Weight for high-frequency clean-latent reconstruction.")
    parser.add_argument("--easyvfx_temporal_weight", type=float, default=0.0, help="Weight for temporal high-frequency consistency.")
    parser.add_argument("--easyvfx_descriptor_weight", type=float, default=0.0, help="Weight for normalized frequency-energy descriptor matching.")
    parser.add_argument("--easyvfx_adaptive_weight", type=float, default=0.0, help="Weight for sample-adaptive frequency routing loss.")
    parser.add_argument("--easyvfx_adaptive_temperature", type=float, default=0.7, help="Temperature for target-frequency-energy routing.")
